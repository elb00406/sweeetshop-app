...
export class DeliveryInfo extends Component {
    constructor(parent: HTMLElement) {
        ...
        new Component(this.node, "h2", ["delivery-title"], "Оплата и доставка");
        ...
        const deliveryOptions = new Component(this.node, "div", ["delivery-options"]);
        ...
        infoBlocks.forEach((info) => {
            const block = new Component(deliveryOptions.node, "div", ["info-block"]);
            new Component(block.node, "div", [info.icon]);
            new Component(block.node, "p", ["info-text"], info.text);
            new Component(
                block.node,
                "img",
                ["product-image"],
